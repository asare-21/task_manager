...
function durationToLogString(duration) {
    const millis = Math.floor(duration.nanos / 1000000);
    if (millis > 0) {
 ...
 * @param channelId
 */
function getHash(hashPolicies, metadata, channelId) {
    let hash = null;
    for (const policy of hashPolicies) {
        let newHash = null;
        switch (policy.type) {
            case 'CHANNEL_ID':
                newHash = xxhash_1.xxhashApi.h64(`${channelId}`, 0n);
                break;
            case 'HEADER': {
                ...
                newHash = xxhash_1.xxhashApi.h64(rewrittenHeaderString, 0n);
                break;
            }
        }
        if (hash === null) {
    ...
    if (hash === null) {
        return xxhash_1.xxhashApi.h64(`${Math.random()}`, 0n);
    }
    else {
        return hash;
    }
...
class SingleClusterRouteAction {
    constructor(cluster, methodConfig, extraFilterFactories, hashPolicies) {
        this.cluster = cluster;
        this.methodConfig = methodConfig;
        this.extraFilterFactories = extraFilterFactories;
...
class WeightedClusterRouteAction {
    constructor(clusters, totalWeight, methodConfig, hashPolicies) {
